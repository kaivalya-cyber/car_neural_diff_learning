...
def worker(remote, parent_remote, sensor_count=16, obstacle_count=0):
    parent_remote.close()
...
class VectorEnv:
    ...
    """
    def __init__(self, num_envs=64, sensor_count=16, obstacle_count=0):
        self.num_envs = num_envs
        self.remotes, self.work_remotes = zip(*[mp.Pipe() for _ in range(num_envs)])
        
        self.processes = []
        for work_remote, remote in zip(self.work_remotes, self.remotes):
            p = mp.Process(target=worker, args=(work_remote, remote, sensor_count, obstacle_count))
            p.daemon = True
            p.start()
