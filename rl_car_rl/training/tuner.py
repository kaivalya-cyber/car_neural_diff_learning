...
def run_tuning_session(config, run_id):
    ...
    final_reward = np.mean(recent_rewards[-50:]) if len(recent_rewards) >= 50 else np.mean(recent_rewards)
    print(f"[{run_id}] Finished! Reward: {final_reward:.2f} | Curriculum Level: {curriculum.level:.2f}")
...
def tune_hyperparameters():
    ...
    print(f"Tuning complete! Saved {len(df)} configurations to {out_path}.")
